...
use meetingrelay_model_worker_sherpa_native::{
    NativeCandidateExecutionInput, NativeCandidateFaultMode, run_locked_native_candidate_fault,
};
...
fn run() -> Result<(), &'static str> {
    let (mode, marker_path, paths) = parse_arguments(std::env::args_os().skip(1))?;
    ...
    run_locked_native_candidate_fault(
        mode,
        &marker_path,
        NativeCandidateExecutionInput {
            executable_path,
            schema_registry_path,
            model_path,
            tokens_path,
...
fn parse_arguments(
...
) -> Result<(NativeCandidateFaultMode, PathBuf, [PathBuf; 7]), &'static str> {
    ...
    let mode = values[0]
        .to_str()
        .ok_or("SHERPA_CONFORMANCE_USAGE")?
        .parse()
        .map_err(|()| "SHERPA_CONFORMANCE_USAGE")?;
    let marker_path = PathBuf::from(&values[1]);
    let paths: Vec<_> = values[2..].iter().map(PathBuf::from).collect();
    let paths = paths.try_into().map_err(|_| "SHERPA_CONFORMANCE_USAGE")?;
    Ok((mode, marker_path, paths))
}
